...
file_server="https://<<FILE SERVER>>/downloads" 	# this could be any file share or S3 bucket or SSC server for example "http://storage.dropbox.com/"

#SSC Server to Upload results
ssc_server="http://<<SSC SERVER>>:<<PORT>>/ssc/" # this could be SSC server address with trailing slash('/')
auth_token="<<CI_TOKEN>>"						 # authentication token for SSC, this should be CI_TOKEN
app_version_id="<<APP VERSION ID>>"				 # application version id from SSC
...
$install_dir/bin/sourceanalyzer -b aws-dotnet-build -clean

echo "Translating..."
#include more file types as required 
$install_dir/bin/sourceanalyzer -b aws-dotnet-build -debug-verbose -logfile aws-dotnet-build.log -dotnet-version 3.1 "./**/*"

echo "Scan Started..."
#Generate SAST report
$install_dir/bin/sourceanalyzer -b aws-dotnet-build -scan -f aws-sast-report.fpr
