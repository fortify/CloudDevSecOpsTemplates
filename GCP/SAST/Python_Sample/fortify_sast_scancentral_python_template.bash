...
#Patch needed if GCP docker version could not pre-req JDK
apt-get -y update
...
echo 'here is the url: '$sc_server
...
scancentral -url $sc_server start --build-tool none --python-requirements requirements.txt -uptoken $auth_token -upload --application $app_name --application-version $app_version
e=$?        # return code last command
if [ "${e}" -ne "0" ]; then
